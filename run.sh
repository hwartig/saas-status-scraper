...
awk -v FS='\t' -v OFS='' '($4=="instatus") {print "echo ",$1,"; mkdir -p data/",$1,"; curl -s -f ",$3," | hxselect -c \"#__NEXT_DATA__\" | jq \".props.pageProps | del(.startDate,.endDate)\" > data/",$1,"/data.json"}' companies.tsv | bash
echo "done"
